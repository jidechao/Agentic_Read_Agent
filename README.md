...
源文档目录 (data/)                    编译产物 (compiled_library/)
  ...
  新增/变更文件     跳过未变文件       <cluster>/INDEX.md
     │                                       │
     ▼                                       ▼
  解析 ──▶ 分类 ──▶ 嵌入 ──▶ 混合聚类  pageindex_cache/
                 │        │              short_docs_db.json
                 │        │              CLUSTER_REPORT.md
                 │        │
                 │        └─ LLM主题摘要 + 净化正文 + L2归一化
                 │
                 └─ 短文档直读 / 长文档 PageIndex 树索引
                               ▼
                         原子物化 (tmp→bak→rename)
```
...
| `src/compiler.py` | 增量编译引擎：扫描 → 解析 → 分类 → 嵌入 → 混合聚类 |
| `src/clustering/` | 聚类子系统：余弦粗聚、LLM 命名、增量分配、质量评估 |
| `src/materializer.py` | 原子物化：tmp→bak→rename 写入 compiled_library/ |
| `src/registry.py` | SQLite 文档注册表：状态机 + 嵌入缓存 + 稳定类目 + 编译追踪 |
...
| `CLUSTER_COSINE_THRESHOLD` | `CLUSTER_COSINE_THRESHOLD` | `0.30` | 余弦层次聚类的距离阈值。越小越容易拆分，越大越容易合并 |
| `INCREMENTAL_ASSIGN_THRESHOLD` | `INCREMENTAL_ASSIGN_THRESHOLD` | `0.55` | 增量文档分配到已有类目的最低余弦相似度 |
| `PENDING_REVIEW_REBALANCE_RATIO` | `PENDING_REVIEW_REBALANCE_RATIO` | `0.10` | 待复核文档占比超过该值时应触发再平衡 |
| `PENDING_REVIEW_REBALANCE_ABSOLUTE` | `PENDING_REVIEW_REBALANCE_ABSOLUTE` | `30` | 待复核文档绝对数量阈值 |
| `PENDING_REVIEW_REBALANCE_MIN` | `PENDING_REVIEW_REBALANCE_MIN` | `5` | 待复核文档少于该值时不触发全量再平衡，避免小语料新增 1-2 篇就重洗类目 |
| `CATEGORY_REUSE_THRESHOLD` | `CATEGORY_REUSE_THRESHOLD` | `0.65` | 全量重建时新旧类目 centroid 相似度达到该值即复用旧类目名称、显示名和描述 |
| `LLM_SUMMARY_MAX_CHARS` | `LLM_SUMMARY_MAX_CHARS` | `80` | LLM 主题摘要最大长度 |
| `TOPIC_SUMMARY_MODEL` | `TOPIC_SUMMARY_MODEL` | `CLUSTER_NAMING_MODEL` | 生成主题摘要使用的模型 |

> **混合聚类**：系统不再使用 KMeans + silhouette 自动 K。现在先用 LLM 主题摘要净化 embedding 输入，再做 L2 归一化和余弦层次粗聚；随后用 LLM 命名、复核明显的小簇合并，并将稳定类目写入 SQLite。全量重建时会优先按旧 `canonical_name`、centroid 相似度和文档重叠复用旧类目的 `canonical_name` / `display_name` / `description`，避免 LLM 命名漂移导致目录改名。
...
# 查看当前聚类质量报告
python -m src.triggers.cli eval
```
...
稳定类目已经存在且没有删除文件时，增量编译会优先把新增/变更文档分配到最近的已有类目；低置信文档进入复核路径。只有待复核文档数量同时达到 `PENDING_REVIEW_REBALANCE_MIN`，且满足占比阈值或绝对数量阈值时，才会触发全量再平衡，避免小知识库新增 1-2 篇新主题文档就重洗全部类目。

全量再平衡（包括 `--force` 或删除文件触发的重建）不会直接采用 LLM 新生成的类目名覆盖旧类目。编译器会在重建前读取旧类别快照，并按以下优先级复用旧类别身份：

1. 新旧 `canonical_name` 相同：直接复用旧 `canonical_name`、`display_name` 和 `description`。
2. `canonical_name` 不同但 centroid 相似度 ≥ `CATEGORY_REUSE_THRESHOLD`：复用旧三件套。
3. 旧 centroid 不可用或不命中时，如果新簇与某个旧类目的文档重叠达到多数，也复用旧三件套。

只有确认是全新主题且无法匹配旧类目时，才使用 LLM 生成的新类目名。
...
├── CLUSTER_REPORT.md         # 聚类质量报告（簇大小、相似度、可疑文档）
...
稳定类目表的重建也使用 SQLite 事务保护：LLM 命名和类目复用先在内存中完成，随后 `clear_categories`、`upsert_category` 和文档类目关联写入在同一事务内提交；如果写库阶段失败，会回滚到重建前状态，避免 categories 表被清空后半途失败。
...
| Embedding | Qwen3-Embedding-4B (1536 维) | 文档向量化、余弦相似度、增量分配 |
...
| 聚类 | scikit-learn AgglomerativeClustering + LLM 复核 | 文档自动分类、稳定类目、质量报告 |
| 注册表 | SQLite | 文档状态管理、嵌入缓存、稳定类目、主题摘要缓存 |
...
当前聚类已支持 LLM 主题摘要、余弦粗聚、稳定类目和小簇合并，但业务方仍可能希望强制某些文档进入固定目录。

**计划**：在 `data/` 目录下支持 `clusters.yaml` 配置文件，允许用户预定义聚类名称和关键词匹配规则。编译时优先按规则匹配，未匹配的文档再走自动聚类。兼顾灵活性和可控性。

### 2. 查询意图预分类

当前 Agent 需要先读 SKILL.md 再逐个检查 INDEX.md，多轮工具调用才能定位文档。

**计划**：在 Agent 路由前增加轻量意图分类步骤（基于关键词或小型分类器），直接将查询映射到最可能的 1-2 个聚类目录，跳过 SKILL.md 全量扫描。

### 3. 文档去重与版本管理

当前基于 content hash 检测重复，但无法处理内容微调（如政策更新）导致的近似重复。

**计划**：引入 embedding 相似度阈值检测近似文档，编译时标记为新版本而非新文档，INDEX.md 中展示版本关系。
