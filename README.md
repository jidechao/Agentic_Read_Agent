...
> **混合聚类**：系统不再使用 KMeans + silhouette 自动 K。现在先用 LLM 主题摘要净化 embedding 输入，再做 L2 归一化和余弦层次粗聚；随后用 LLM 命名、复核明显的小簇合并，并将稳定类目写入 SQLite，避免每次编译后目录身份漂移。
...
稳定类目已经存在且没有删除文件时，增量编译会优先把新增/变更文档分配到最近的已有类目；低置信文档进入复核路径，避免每次小改动都全量重聚类。
