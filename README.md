...
当前聚类完全依赖 embedding 相似度自动分组，当文档数量少或主题分布不均时，可能出现不合理分类。

**计划**：在 `data/` 目录下支持 `clusters.yaml` 配置文件，允许用户预定义聚类名称和关键词匹配规则。编译时优先按规则匹配，未匹配的文档再走自动聚类。兼顾灵活性和可控性。

### 2. 基于 LLM 的文档摘要生成

当前 INDEX.md 摘要是纯提取（章节标题 + 正文首句），信息密度有限。

**计划**：在编译阶段调用 LLM 为每个文档生成 2-3 句高质量摘要，覆盖核心内容和适用场景。摘要同时用于 INDEX.md 展示和 embedding 输入，提升聚类和导航两方面的准确性。

### 3. 增量重聚类

当前每次编译如果有文档变更，会对所有文档重新聚类和重写 INDEX.md。

**计划**：对已编译且内容未变的文档保留原有聚类归属，仅对新文档分配到最近聚类或触发局部重聚类。减少编译时间和 API 调用开销。
